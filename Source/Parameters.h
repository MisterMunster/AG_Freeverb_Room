...
// Toggles/Mode
static const juce::String mode =
    "mode"; // 0 = Early, 1 = Late (for UI, but maybe processing too?)
...
// I will assume both always run, and the UI toggles which set of fine-tune
// knobs are visible. Though "Early Send" controls how much goes to Late.

static const juce::String earlySend =
    "early_send"; // How much of Early goes to Late
...
createParameterLayout() {
  ...
  auto makeParam = [&](const juce::String &id, const juce::String &name,
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        id, name, juce::NormalisableRange<float>(min, max, 0.01f, skew), def));
  };

  makeParam(mix, "Mix", 0.0f, 100.0f, 100.0f);
  makeParam(predelay, "Predelay", 0.0f, 200.0f, 10.0f,
            0.5f); // Skew for exp feel
  makeParam(decay, "Decay Time", 0.1f, 10.0f, 2.0f, 0.5f);
  makeParam(loCut, "Lo Cut", 20.0f, 500.0f, 20.0f, 0.5f);
  makeParam(hiCut, "Hi Cut", 1000.0f, 20000.0f, 3500.0f, 0.5f);
  makeParam(modDepth, "Mod Depth", 0.0f, 100.0f, 15.0f);

  makeParam(earlySize, "Early Size", 10.0f, 500.0f, 100.0f, 0.5f);
  ...
  makeParam(diffusion, "Diffusion", 0.0f, 1.0f, 0.6f);
  makeParam(earlySend, "Early Send", 0.0f, 1.0f,
            0.0f); // Default 0 means parallel? Request says "Controls amount of
                   // early sent to late". Usually cascade.
  // If 0, late gets Dry input?
  // I'll implement logic: Late Input = Dry * (1-EarlySend) + Early * EarlySend
  // ? Or usually Late is fed by Early. "Default: 0.00" suggests by default Late
  // is fed by Dry directly (Parallel).
