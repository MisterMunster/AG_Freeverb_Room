...
int AntigravReverbAudioProcessor::getCurrentProgram() {
  return 0; // Not tracking actual state index effectively without extra logic,
            // keeping simple
}
...
void AntigravReverbAudioProcessor::changeProgramName(
    int /*index*/, const juce::String & /*newName*/) {}

//==============================================================================
void AntigravReverbAudioProcessor::prepareToPlay(double sampleRate,
                                                 int /*samplesPerBlock*/) {
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool AntigravReverbAudioProcessor::isBusesLayoutSupported(
...
void AntigravReverbAudioProcessor::processBlock(
  ...
  // Scale Late Reverb Size based on Early Size (300ms = 1.0)
  // Range roughly 0.2 to 2.0
  float sizeScaler = earlySizeMs / 300.0f;
  if (sizeScaler < 0.2f)
    sizeScaler = 0.2f; // Clamp minimum to avoid artifacts
  if (sizeScaler > 2.5f)
    sizeScaler = 2.5f;

  lateReverb.setParameters(decayS, subModDepth * modDepthVal, modRateVal,
                           hiCutHz, loCutHz, sizeScaler, diffusionVal);
...
//==============================================================================
bool AntigravReverbAudioProcessor::hasEditor() const { return true; }
...
//==============================================================================
void AntigravReverbAudioProcessor::getStateInformation(
...
void AntigravReverbAudioProcessor::setStateInformation(const void *data,
                                                       int sizeInBytes) {
